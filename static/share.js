        ...
        async function generateShareCard() {
            ...
            if (uploadedFile && previewImage.style.display !== 'none') {
                source = previewImage;
                srcWidth = previewImage.naturalWidth;
                srcHeight = previewImage.naturalHeight;
            } else {
                // If using video, ensure we capture the current frame
                // Note: videoWidth might be 0 if not ready, but we are at result screen so it should be fine.
            }
            ...
            let line = '';
            let lineY = shareCanvas.height - 250;
            const maxWidth = shareCanvas.width - 100;
            const lineHeight = 70;

            // Simple multi-line simple wrapper for canvas
            // Since captions are short one-liners, we might just need 2 lines max usually.
            // But let's be safe.
            
            // Actually, let's just draw it.
            // For simplicitly, assume brief captions fit or wrap.
